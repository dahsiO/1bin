...
//Ajouter un setter pour le numéro de téléphone.
    ...
    //Cette méthode renvoie false si le paramètre est null ou si le membre a déjà un parrain ou si le membre passé en paramètre est le membre lui-même.
    //Sinon, elle initialise le parrain et renvoie true.


    public boolean Parrain(Membre parrain) {
        //parametre == null  : false
        if (parrain == null){
            return false;
        }
        // membre est lui meme parrain : false
        if (this == parrain){
            return false;
        }
        //membre a deja un parrain  : false
    ...
    public String toString() {
        ...
        String info = prenom + " " + nom + " (" + numero + ")";
        if (parrain == null) {
            info += " | Pas de parrain.";
        } else {
            info += " | Parrain : " + parrain.getPrenom() + " " + parrain.getNom();
        }
        return info;
    }
